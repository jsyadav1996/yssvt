...
import { apiClient, Donation, User as UserType } from '@/lib/api'

const DonationEditPage: React.FC = () => {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          setFormData({
            ...
            date: donationData.date.split('T')[0], // Convert to date-only format
            location: donationData.location || '',
            bankName: donationData.bankName || ''
          })

          // Set donor information if exists
          if (donationData.donorId && response.data.donor) {
            const donorInfo = {
              id: donationData.donorId,
              firstName: response.data.donor.firstName,
              lastName: response.data.donor.lastName,
            ...
            setSearchQuery(`${response.data.donor.firstName} ${response.data.donor.lastName}`)
          }
        } else {
    ...
    return (
      <div className="space-y-4 sm:space-y-6">
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Edit Donation</h1>
          </div>
        </div>
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Edit Donation</h1>
          <p className="text-sm sm:text-base text-gray-600">Update donation information</p>
        </div>
      </div>
